...
# ==================================
"""
Centralized background task execution utilities - FIXED VERSION
...
from app.api.utils.progress_tracking import ProgressTracker, SingleStepProgressTracker, force_cleanup_session

# Get logger
logger = logging.getLogger(__name__)

# =============================================================================
# BACKGROUND EXECUTION FUNCTIONS - FIXED VERSION
# =============================================================================
def _execute_individual_asce_summary_with_progress(session_id, project_data, address, asce_options):
    """
    Execute individual ASCE Summary with progress tracking - FIXED VERSION
    Note: This runs in a background thread without Flask request context.
    """
    # CRITICAL FIX: Force cleanup any existing session data first
    force_cleanup_session(session_id)
    
    # Initialize tracker with clean state
    tracker = SingleStepProgressTracker(session_id, 'ASCE Summary')
    project_number = project_data['code']
    
    try:
        logger.info(f"Starting ASCE Summary generation for project {project_number} (Session: {session_id})")
        
        # Step 1: Check if AHJ Report exists - START WITH 0% PROGRESS
        tracker.update_progress(0, 'Checking AHJ report availability...', 'in_progress')
        ...
        if not workbook_exists:
            ...
            tracker.update_progress(20, 'Creating AHJ report...', 'in_progress')
        ...
        tracker.update_progress(40, 'Initializing ASCE summary scraper...', 'in_progress')
        scraper = ASCEScraper()
        
        try:
            # Step 3: Execute scraping
            tracker.update_progress(60, 'Extracting ASCE summary data...', 'in_progress')
            ...
            tracker.update_progress(80, 'Saving summary to AHJ report...', 'in_progress')
            ...
            tracker.update_progress(90, 'Preparing summary display...', 'in_progress')
            
            # Store summary data in the progress tracker for retrieval
            summary_dict = {
                'wind_data': {
...
def _execute_individual_asce_report_with_progress(session_id, project_data, address, asce_options):
    """Execute individual ASCE report with progress tracking - FIXED VERSION"""
    # CRITICAL FIX: Force cleanup any existing session data first
    force_cleanup_session(session_id)
    ...
    try:
        logger.info(f"Starting ASCE Full Report generation for project {project_number} (Session: {session_id})")
        
        # Ensure project directory exists - START WITH 0% PROGRESS
        tracker.update_progress(0, 'Setting up project directories...', 'in_progress')
        ...
        tracker.update_progress(10, 'Starting ASCE report generation...', 'in_progress')
...
def _execute_individual_soil_survey_with_progress(session_id, project_data, address):
    """Execute individual soil survey with progress tracking - FIXED VERSION"""
    # CRITICAL FIX: Force cleanup any existing session data first
    force_cleanup_session(session_id)
    ...
    try:
        logger.info(f"Starting USDA Soil Survey generation for project {project_number} (Session: {session_id})")
        ...
        tracker.update_progress(30, 'Starting soil survey generation...', 'in_progress')
...
def _execute_unified_scraping_with_progress(session_id, project_data, address, asce_options):
    """
    Execute all scraping operations with real progress tracking - FIXED VERSION
    """
    # CRITICAL FIX: Force cleanup any existing session data first
    force_cleanup_session(session_id)
...
# Rest of the functions remain the same but with improved progress updates...
def _execute_asce_full_report_with_progress(project_number, address, asce_options, project_dir, tracker, step_index):
    """Execute ASCE Full Report with progress updates - IMPROVED PROGRESS GRANULARITY"""
    try:
        # Set up file paths
        ...
        if isinstance(tracker, SingleStepProgressTracker):
            tracker.update_progress(20, 'Initializing ASCE scraper...', 'in_progress')
        else:
            tracker.update_step(step_index, 'in_progress', 20, 'Initializing ASCE scraper...')
        ...
        try:
            ...
            if isinstance(tracker, SingleStepProgressTracker):
                tracker.update_progress(30, 'Accessing ASCE website...', 'in_progress')
            else:
            ...
            if isinstance(tracker, SingleStepProgressTracker):
                tracker.update_progress(80, 'Saving ASCE report...', 'in_progress')
            else:
            ...
            if isinstance(tracker, SingleStepProgressTracker):
                tracker.update_progress(95, 'ASCE report saved successfully', 'in_progress')
            else:
...
def _execute_soil_survey_with_progress(project_number, address, project_dir, tracker, step_index):
    """Execute Soil Survey with progress updates - IMPROVED PROGRESS GRANULARITY"""
    temp_download_dir = None
    
    try:
        # Set up directories
        if isinstance(tracker, SingleStepProgressTracker):
            tracker.update_progress(25, 'Setting up soil survey directories...', 'in_progress')
        else:
            tracker.update_step(step_index, 'in_progress', 25, 'Setting up soil survey directories...')
        ...
        if isinstance(tracker, SingleStepProgressTracker):
            tracker.update_progress(35, 'Initializing soil survey scraper...', 'in_progress')
        else:
            tracker.update_step(step_index, 'in_progress', 35, 'Initializing soil survey scraper...')
        ...
        try:
            if isinstance(tracker, SingleStepProgressTracker):
                tracker.update_progress(55, 'Accessing USDA soil survey website...', 'in_progress')
            else:
                tracker.update_step(step_index, 'in_progress', 55, 'Accessing USDA soil survey website...')
            ...
            if isinstance(tracker, SingleStepProgressTracker):
                tracker.update_progress(80, 'Processing soil survey reports...', 'in_progress')
            else:
            ...
            if isinstance(tracker, SingleStepProgressTracker):
                tracker.update_progress(95, f'Saved {len(saved_paths)} soil reports', 'in_progress')
            else:
...
def _execute_asce_summary_with_progress(project_number, address, asce_options, tracker, step_index):
    """Execute ASCE Summary with progress updates - IMPROVED PROGRESS GRANULARITY"""
    try:
        if isinstance(tracker, SingleStepProgressTracker):
            tracker.update_progress(10, 'Checking AHJ report availability...', 'in_progress')
        else:
        ...
        if isinstance(tracker, SingleStepProgressTracker):
            tracker.update_progress(30, 'Initializing ASCE summary scraper...', 'in_progress')
        else:
        ...
        try:
            if isinstance(tracker, SingleStepProgressTracker):
                tracker.update_progress(50, 'Extracting ASCE summary data...', 'in_progress')
            else:
            ...
            if isinstance(tracker, SingleStepProgressTracker):
                tracker.update_progress(95, 'ASCE summary saved to AHJ report', 'in_progress')
            else:
